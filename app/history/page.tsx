  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (!user) {
        ...
        try {
          const res = await fetch(`/api/farm-history?userId=${user.uid}`);
            if (res.ok) {
            ...
            history.map((plan) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase text-secondary/40 tracking-widest">Plan #{plan.id.slice(0, 6)}</span>
                ...
                <h4 className="font-bold text-primary">{plan.farmPlan?.market_strategy?.action || "Generated Strategy"}</h4>
                <p className="text-xs text-secondary/60 line-clamp-2">{plan.farmPlan?.farm_summary?.key_issue}</p>
              </Card>
            ))
          )}
        </div>
